...
class RegularMatch(models.Model):
    result = models.CharField(max_length=32, verbose_name='结果名称')
    regular = models.CharField(max_length=1024, verbose_name='正则表达式')
    ...
    omittag = models.CharField(max_length=1024, verbose_name='去除标记')#去除HTML标记，只保留内容： ['font','/font']表示匹配前过滤掉'font','/font'两个tag
    

class PageInfo(models.Model):
    url = models.TextField(verbose_name='页面URL/源码')
    regular_matchs = models.ManyToManyField(RegularMatch, blank=True, verbose_name='正则匹配' )   # style_fields=['m2m_transfer' , ]
    block_matchs = models.ForeignKey(BlockMatch)   #
